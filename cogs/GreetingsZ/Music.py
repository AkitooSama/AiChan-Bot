...
class MusicBot(commands.Cog):
    ...
    @discord.app_commands.command(name="resetfilter",description = "Resets the applied filter")
    async def resetfilter(self,interaction: discord.Interaction):
        guild_info = self.get_guild_info(interaction.guild.id)
        await guild_info['vc'].set_filter(wavelink.Filter(karaoke=None, tremolo=None, vibrato=None, distortion=None, low_pass=None))
        await interaction.response.send_message("Filter Reset Successful! 📼")
        
    @discord.app_commands.command(name="filter", description="add filter to the song")
    @discord.app_commands.describe(filter_name="Name of the song.")
    async def filter(self, interaction: discord.Interaction, filter_name:Choice[str]):    
        guild_info = self.get_guild_info(interaction.guild.id)
        filters = {
            "Karaoke": wavelink.Karaoke(),
            "Vibrato": wavelink.Vibrato(),
            "Distortion": wavelink.Distortion(scale=2),
            "Low_pass": wavelink.LowPass(),
            "Tremolo": wavelink.Tremolo(),
        }

        await guild_info['vc'].set_filter(wavelink.Filter(**{filter_name.lower(): filters.get(filter_name, None)}))
        #self.delaytime = 2
        await interaction.response.send_message(f"Filter: {filter_name} ✅")
    ...
    #@discord.app_commands.describe(name ="Name of the song.")
    #@discord.app_commands.autocomplete(song_name=fruit_autocomplete)
    ...
    async def play(self, interaction: discord.Interaction, song_name: str):
        ...
        if guild_info['vc'].playing:
            await guild_info['vc'].stop()
        print("hohasjis")
        tracks = await wavelink.Playable.search(song_name, source=wavelink.TrackSource.YouTube)
        if not tracks:
